...
3. 复位开发板，确保系统工作在BOOT模式，详细请参考：[关于钛极OS(TiJOS)系统工作模式](../../tijos-development-guide/tijos_workingmode.md)
...
| 存储特性     | 支持    | 说明          |
| -------- | ----- | ----------- |
| 支持应用数据存储 | 128KB | JAVA文件系统空间  |
| 支持应用程序存储 | 256KB | *.TAPK执行区空间 |
| 支持应用程序内存 | 16KB  | *.TAPK执行堆空间 |

| 电源管理特性             | 支持   | 说明          |
| ------------------ | ---- | ----------- |
| 支持冬眠1，指定时间自动唤醒     | 支持   | **最大4200秒** |
| 支持冬眠2，指定GPIO的PIN唤醒 | 不支持  | 无           |
| 支持待机，指定GPIO的PIN唤醒  | 不支持  | 无           |
